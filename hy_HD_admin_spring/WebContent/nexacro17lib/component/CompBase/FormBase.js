...
if (!nexacro.Form) {
	...
	_pForm.go = function (v) {
		if (this._orgurl != v) {
			...
			this.parent.url = this._orgurl = v;
	...
	_pForm.reload = function () {
		...
		if (!this._orgurl) {
			this._orgurl = this.parent._formurl;
		}
		this._base_url = nexacro._getBaseUrl(this._orgurl);
		this._apply_formurl();
	};
	...
	_pForm._apply_formurl = function () {
		this._clear();
		if (this._orgurl) {
			this.loadForm(this._orgurl, this._async, true);
